...
**Where to enter it**: When you first launch the app, the dashboard panel opens with a settings screen. Paste your key there and hit Save. It's stored in the browser's `localStorage` — stays on your machine, never sent anywhere except OpenAI's API.
...
│   ├── hooks/              # useCamera, useVoice, useTTS
│   ├── services/           # STT, TTS, AI streaming, context builder, compositor
│   ├── contexts/           # Global app state (AppContext)
│   ├── lib/                # Utils, constants, tutoring prompts
│   └── types/              # TypeScript interfaces
├── src-tauri/              # Rust backend
│   └── src/
│       ├── lib.rs          # App init, NSPanel setup
│       ├── api.rs          # OpenAI API (Whisper, GPT-4o, TTS)
...
- [Project Plan](docs/PROJECT_PLAN.md) — problem definition, user stories, scope, risks
- [Architecture](docs/ARCHITECTURE.md) — system design, data flow, technology decisions
- [Master Implementation Plan](docs/MASTERPLAN.md) — detailed phase-by-phase build plan
